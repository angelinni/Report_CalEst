...
def est1_fechas(red,est_,sens_):
    
    
    
    #Selección de Estacion
    all_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Datos/{red}/Tablas_Cal_tiempos/*")
    #print(all_est)
    estaciones = []
    for est in all_est:
        nom_est = est.split("/")[-1].split("_")[0]
    ...
    with col2:
        list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Datos/{red}/Tablas_Cal_tiempos/{option}*")
        ...
        list_est_sen = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Datos/{red}/Tablas_Cal_tiempos/{option}_{option_sen}Z*")
    ...
    #SELECCION DE FECHAS
    mes_antes=datetime.now()-timedelta(days=32)
    ...
    if red == "LB":
        ...
        #TITULO
        st.header("""Funcionamiento y calidad de señal""")
        st.header("""En servidor de pruebas .50""")
        st.header(option+" _ "+option_sen+"."+option_cod_sen)
    ...
    with tabz:
        ...
        if option_cod_sen == ".":
            list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Datos/{red}/Tablas_Cal_tiempos/{option}_{option_sen}{com}*")
        if len(option_cod_sen) > 1:
            list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Datos/{red}/Tablas_Cal_tiempos/{option}_{option_sen}{com}_{option_cod_sen}*")
        ...
        with col2:
                ...
                plot_CalEst.plot_ruido(fecha,option,option_sen, com,red)
        with col3:
            st.write(".")
...
def estado_est():
    ...
    st.write("Estado Actual de las estaciones")

    plot_CalEst.create_db_all_state()
...
def main():
    ...
    #Tipo de Consulta
    option_consulta = st.sidebar.selectbox('**Consulta**',["Por fechas","Informe semestral","Informe mensual","Estado Actual"] ,key ="cons")
    
    st.sidebar.write("_________")

    

    #if option_consulta == "CM":

    if option_consulta == "Por fechas":
        
        #RED
        option_red = st.sidebar.selectbox('**Red**',["CM","LB"] ,key ="redes")
        est1_fechas(option_red,"no","no")
    ...
    if option_consulta == "Estado Actual":
        estado_est()
